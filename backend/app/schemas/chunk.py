...
class ChunkResponse(ChunkBase):
    ...
    embedding_model: str | None = Field(default=None, description="The name of the embedding model.")
    embedding_version: str | None = Field(default=None, description="The pipeline schema version used.")
    embedded_at: datetime | None = Field(default=None, description="UTC timestamp of vector execution.")
    embedding_duration_ms: int | None = Field(default=None, description="Duration in ms.")
...
class ChunkEmbeddingResponse(BaseModel):
    ...
    id: uuid.UUID
    embedding: list[float] | None = Field(default=None, description="The float vector array elements.")
...
class DocumentEmbeddingStatusResponse(BaseModel):
    ...
    processing_time_ms: int = Field(description="Estimated processing time elapsed or total.")
...
class DocumentEmbeddingSummaryResponse(BaseModel):
    ...
    document_id: uuid.UUID
    total_embedded: int = Field(description="Total count of chunks indexed with vectors.")
    vector_dimension: int = Field(description="Dimensionality size (e.g. 768).")
